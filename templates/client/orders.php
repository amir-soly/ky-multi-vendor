...
$seller_orders = get_orders_by_seller_id($seller_id);

// test_export_orders_to_excel();
...
<div>
    ...
    <div>
        ...
        <div class="flex-cb mb-6">
            <button id="get_orders_export" class="text-paragraph text-xs btn-flex bg-white border border-[#39B54A] rounded-full py-2.5 px-6">
        ...
        <table class="border-collapse table-fixed w-full">
            ...
            <?php
    $order_counter = 1;
    foreach ($seller_orders as $order_id => $order_data): ?>
        <tr class="even:bg-back/30 border-b last:border-b-0">
            <td class="p-3 text-center font-light" data-label="ردیف"><?php echo $order_counter++; ?></td>
            <td class="p-3 text-center" data-label="کد سفارش">#<?php echo $order_id; ?></td>
            <td class="p-3 text-center" data-label="تاریخ ثبت‌ سفارش"><?php echo $order_data['date']; ?></td>
            <td class="p-3 text-center" data-label="وضعيت"><span class="w-full block rounded-full py-2 text-center text-green-700 bg-green-700/25"><?php echo $order_data['status']; ?></span></td>
            <td class="p-3 text-center" data-label="مجموع"><?php echo number_format($order_data['total_amount']) . ' تومان'; ?></td>
            <td class="p-3"><a href="<?php echo get_permalink($order_id); ?>" class="bg-secondary w-full block rounded-full text-center text-white py-2">نمایش</a></td>
        </tr>
    <?php endforeach; ?>
            </tbody>
        </table>
